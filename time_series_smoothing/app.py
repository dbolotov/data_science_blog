...
with left_col:
    ...
    with col1:
        ...
        ma_window = st.slider(
            ...
            key="ma",
            help=(
                "**Moving Average (Rolling Mean)**: Replaces each point with the average of its neighbors.\n"
                "Good for quick trend smoothing and removing short-term noise. Limitations: lags behind sharp changes and flattens peaks.\n\n"
                "**Window**: how many data points to average at each step."
            ),
        )
    ...
    with col2:
        ...
        ema_alpha = st.slider(
            ...
            key="ema",
            help=(
                "**Exponential Moving Average (EMA)**: Similar to moving average but gives more weight to recent values.\n"
                "Good for tracking recent trends while still smoothing.\n"
                "Limitations: lags behind sharp changes, results depend heavily on the alpha setting.\n\n"
                "**Alpha**: smoothing factor that determines how much weight is given to recent data versus older data."
            ),
        )
    ...
    with col3:
        ...
        sg_window = st.slider(
            ...
            step=2,
            key="sg_win",
            help="**Savitzky–Golay Filter**: Fits a polynomial to a local window.\n"
            ...
            "**Window**: number of points used for each fit (must be odd).\n\n"
            "**Poly**: degree of the polynomial used to fit the window.",
        )
    ...
    with col4:
        ...
        loess_frac = st.slider(
            ...
            key="loess",
            help=(
                "**LOESS (LOWESS)**: Fits a local regression for each data point using neighboring points."
    ...
    with col5:
        ...
        gauss_sigma = st.slider(
            ...
            help=(
                ...
                "Limitations: smoothing can blur sharp transitions and introduces a small lag.\n\n"
    ...
    with col6:
        ...
        kf_transition_noise = st.slider(
            ...
            help=(
                ...
                "Good for dynamic smoothing, handling noisy or missing data, and adapting over time. Limitations: more complex, sensitive to parameter tuning, and assumes linear dynamics.\n\n"
...
df["ma"] = (
    df["noisy_signal"].rolling(window=ma_window, center=True).mean().bfill().ffill()
)
df["ema"] = df["noisy_signal"].ewm(alpha=ema_alpha).mean()
df["savgol"] = savgol_filter(
    df["noisy_signal"], window_length=sg_window, polyorder=sg_poly
)
...
with right_col:
    ...
    tv_orig = total_variation(df["noisy_signal"])
    std_orig = df["noisy_signal"].std()
